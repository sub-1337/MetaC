...
```
Пробует сконструировать объект в месте вызова. 
Вернёт копию или перемещённое значение если это невозможно.

```
fn returnName() -> (let rvo : string)
{
	return "Joe";
}
```
Сконструирует объект в месте вызова или ошибка компиляции.
...
{
	let mut arrayToReturn : int[] = [1,2,3];
...
(let mut numName, let const stringName) = retMult();
```
...
#### Статичные
Статичные множественные параметры обрабатываются в compile time
и не влияют на производительность.
Каждому вызову с определёнными параметрами будет сгенерирован свой код.
...
#### Динамические
Динамические множественные параметры задействуют механизм полиморфизма. Если мы пользуемся ими у нас будет не такая хорошая 
производительность в сравнении с статичными параметрами,
но мы сможем обрабатывать тип переменных в run time.
```
import args::dynamic;
@fn func(@let args : [@...])
{
